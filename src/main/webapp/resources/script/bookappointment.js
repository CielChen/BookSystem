document.write("<script language=javascript src='/booksystem/resources/script/dictionary.js'></script>");
var bookappointment = {
    //封装相关ajax的url
    URL: {
        appoint: function (bookId, studentId) {
            return '/booksystem/books/' + bookId + '/appoint?studentId=' + studentId;
        },
        verifyStudent: function () {
            return '/booksystem/books' + '/verifyStudent';
        },
        verifyAdmin: function () {
            return '/booksystem/books' + '/verifyAdmin';
        },
        addBook: function () {
            return '/booksystem/books' + '/add'
        },
        addBookData: function () {
            return '/booksystem/books' + '/bookData'
        },
        IP: function () {
            return "http://localhost:8080";
        },
        bookList: function () {
            return "/booksystem/books" + "/list"
        },
        adminBookList: function(){
        	return "/booksystem/books" + "/adminlist"
        }
    },
    ...
    list: {
        ...
        sortList: function (sortType) {
            var sortUrl = '/booksystem/books/sort';
            var params = {};
            var column = 1;
                        ...
                        $(function () {
                            for (var i = 0; i < result.length; i++) {
                                $('#bookList').append(
                                    ...
                                    '<td><a class="btn btn-info" href="/booksystem/books/' + result[i].bookId + '/detail" >详细</a></td>' +
                                    '</tr>');
                            }
                            $('#bookList').html();
                        });
        ...
        init: function () {
            ...
            $('#clearCache').click(function () {
                $.cookie('userId', '', {path: '/booksystem/books'});
                $.cookie('password', '', {path: '/booksystem/books'});
            });
            // 排序
            //设置标志位判断当前的顺序是顺序还是逆序
            adescValue = 1;
    ...
    detail: {
        ...
        userLogin: function (userId, password, userType) {
            ...
            if (temp == "nothing") {
                ...
                $.cookie('userId', userId, {path: '/booksystem/books'});
                $.cookie('password', password, {path: '/booksystem/books'});
                $.cookie('userType', userType, {path: '/booksystem/books'});
                
                if(userType == 'admin'){
                	var userUrl = bookappointment.URL.adminBookList();
                	window.location.href =  bookappointment.URL.IP() + userUrl;
                }
            }
        },
    },
